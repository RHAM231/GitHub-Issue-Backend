...
'''
Let's define CRUD functionality for our issues. We use Django's
pre-built class based views, ListView, DetailView, CreateView, and
UpdateView.

Any time we use a view to make changes to our issues, we'll also call
our GitHub issue methods from github_client.py in the sync app.

We'll pull GitHub login information from our settings.py file (in turn
pulled from a config file) and pass the credentials into our GitHub
methods by overwriting form_valid on our edit views.

Issue Tracker will first save our form data to our database, then
create the issue on GitHub, updating our database instance with the
newly created GitHub issue number.
'''
...
class IssueListView(FormMixin, ListView):
    ...
    # Override ListView's get_queryset() method to add simple search to
    # our view
    def get_queryset(self, **kwargs):
        ...
        # If we're visiting the page for the first time, return all our
        # issues
        else:
...
# Define a generic view to display a single issue. Reroute to either a
# detail view or an update view depending on whether we're using get or
# post.
...
# If using get, just display the issue with it's parameters and the
...
class IssueDetailView(DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Create a stamp path parameter from the stamp info to add to
        # context
        if issue.associated_folder is not None:
            issue_path = \
                stamp_value.splitlines()[0].replace('Issue Location: ', '')
        else:
            issue_path = None

        context['issue_path'] = issue_path
        context['form'] = \
            OpenCloseIssueForm(initial={'state': self.object.state })
        return context


# If using post, check whether the issue is open or closed, update it,
# and then return the issue's detail view
...
class IssueCreateView(CreateView):
    ...
    # Overwrite CreateView's form_valid method so we can call our
    # GitHub create_issue method when saving the form
    def form_valid(self, form):
        ...
        # First save the valid form data to create the new object in
        # our database
        ...
        # Now call our create method from our github_client script to
        # create the object in GitHub's database and update our
        # database with the newly created GitHub number
        create_issue(self.token, self.gh_user, data, stamp, issue_id)

        # Now that we've created matching instances in both databases,
        # redirect to our object detail page
        return response
...
# Define a view to load dropdown select options on our issue form. Get
# the object id from AJAX and send back an options queryset to the
# options template.
def load_options(request):
    # If the repository selection changes, return a related folder
    # queryset
    if request.GET.get('repo'):
        instance_id = request.GET.get('repo')
        instances = \
            RepoFolder.objects.filter(repository=instance_id).order_by('name')
    # If the folder changes, get the related files
    elif request.GET.get('folder'):
        instance_id = request.GET.get('folder')
        instances = RepoFile.objects.filter(
            parent_folder=instance_id).order_by('name')
    # If the file changes, get the related lines of code
    elif request.GET.get('file'):
        instance_id = request.GET.get('file')
        instances = LineOfCode.objects.filter(
            repofile=instance_id).order_by('line_number')
    ...
    return render(
        request, 
        'issues/select_dropdown_list_options.html', 
        context
        )
