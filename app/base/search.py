...
def get_search_models(modules, exclusions):
    ...
    for module in modules:
        ...
        # Convert our dictionary to a list for now and filter out
        # duplicates
...
# Given a form from our search view, return a query list based on our
# search term in our navbar search field
def get_search_results(form):
    # Go to our settings file and retrieve our database type
    db_engine = settings.DATABASES['default']['ENGINE']
    # Define our modules and exclusions so we can pass them to our
    # get_search_models method above
    ...
    # Get our actual model query fields from our search models if
    # they're in field_names
    for model in search_models:
        ...
        search_queries = []
        for field in fields:
            # Set search text to pass to query based on active database
            # type For development Sqlite database
            ...
            # Build our queries from our search text, fields, and our
            # search term from the frontend
            q_object = Q(**{
                field.name + search_text : form.cleaned_data['master_search']
                })
            search_queries.append(q_object)
        
        # Chain our field queries together before passing them to
        # filer() below
        q_object = Q()
        for query in search_queries:
            # Check if our search term is in field1 OR field2 OR field3
            # etc.
    ...
    # Sort our search results by model type and date created then
    # return to our structured method below
    results_list = sorted(
        chain.from_iterable(lookups), 
        key=lambda instance: instance.created_at
        )
    return results_list


# Given a form from our search view, call our get_search_results method
# above and split the results by model
